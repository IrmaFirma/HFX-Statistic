...
abstract class Database {
  ...
  Stream<Record> recordStream({@required String recordId});
...
class FirestoreDatabase implements Database {
  ...
  Future<void> deleteRecord(Record record) async {
    // delete where entry.recordId == record.recordId
  ...
  @override
  Stream<Record> recordStream({String recordId}) => _service.documentStream(
      path: APIPath.record(uid, recordId),
      builder: (data, documentId) => Record.fromMap(data, documentId));
  ...
  Stream<List<Entry>> entriesStream({Record record}) =>
      _service.collectionStream<Entry>(
        ...
        queryBuilder: record != null
            ? (query) => query.where('recordId', isEqualTo: record.id)
            : null,
